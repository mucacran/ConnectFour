...
public class GameState
{
	public class GameMove
	{
		...
		public GameMove(int turn, int player, int column, int row)
		{
			Turn = turn;
			Player = player;
			Column = column;
	...
	// nombre de los jugadores game
	public string Player1Name { get; set; } = "Jugador 1";
	public string Player2Name { get; set; } = "Jugador 2";
	public List<GameMove> Moves { get; } = new();
	public int Player1ConsecutiveWins { get; private set; } = 0;
	public int Player2ConsecutiveWins { get; private set; } = 0;
	public bool IsCurrentGameRecorded { get; private set; } = false;
	...
	public byte PlayPiece(int column)
	{
		...
		Moves.Add(new GameMove(currentTurn, currentPlayer, displayColumn, landingRow));
	...
	public void ResetBoard()
	{
		...
		Moves.Clear();
		IsCurrentGameRecorded = false;
	}

	public void RecordGameResult(WinState gameResult)
	{
		if (IsCurrentGameRecorded)
		{
			return;
		}

		if (gameResult == WinState.Player1_Wins)
		{
			Player1ConsecutiveWins++;
			Player2ConsecutiveWins = 0;
			IsCurrentGameRecorded = true;
			return;
		}

		if (gameResult == WinState.Player2_Wins)
		{
